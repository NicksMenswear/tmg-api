...
def update_attendee(attendee_data):
    ...
    try:
        ...
        attendee_detail = (
            db.query(Event).filter(Event.id == attendee_data["event_id"], Attendee.attendee_id == attendee.id).first()
        )
...
def soft_delete_attendee(attendee_data):
    ...
    try:
        ...
        attendee_detail = (
            db.query(Attendee)
            .filter(Attendee.event_id == attendee_data["event_id"], Attendee.id == attendee_data["attendee_id"])
            .first()
        )
