...
def list_attendee(attendee_data):
    ...
    try:
        attendee = db.query(User).filter(User.email == attendee_data['email']).first()
        if not attendee:
            raise abort(404, description="attendee not found")
        ...
        if not attendee_details:
                raise abort(404, description="data not found for this event and attendee")
...
def get_attendees_by_eventid(event_id): 
    ...
    try:
        ...
        if not attendees_detail:
                raise abort(404, description="No Attendee For This Event Found")
        formatted_data = []
