...
EVENT_OWNER_DISCOUNT_VIRTUAL_PRODUCT_PREFIX = "EVENT-OWNER-DISCOUNT"
    ...
    def create_discount_intents(
        ...
        try:
            ...
            shopify_product = self.shopify_service.create_virtual_product(
                ...
                sku=f"{EVENT_OWNER_DISCOUNT_VIRTUAL_PRODUCT_PREFIX}-{str(event.id)}-{datetime.now(timezone.utc).isoformat()}",
                tags=",".join(
                    ["virtual", "event_owner_discount", "event_id=" + str(event.id), "user_id=" + str(event.user_id)]
                ),
            )
