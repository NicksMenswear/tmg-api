...
def test_signup_form(page: Page):
    ...
    email_content = email.look_for_email("Registration email", EMAIL_FROM, user_email, timeout_seconds=120)
...
#
#     email_content = email.look_for_email(
#         "Customer account password reset", None, "e2etmg+011b893a2536@hotmail.com", timeout_seconds=120
#     )
#     assert email_content is not None
#
#     decoded_content = quopri.decodestring(email_content.replace("=\n", "")).decode("utf-8")
#
#     pattern = r"Reset your password.*?\( (https?://[^\s)]+)"
...
#     page.goto(url)
#
#     page.locator("#customer_password").fill("123456")
#     page.locator("#customer_password_confirmation").fill("123456")
#     page.get_by_role("button", name="Reset Password").click()
#
#     ui.access_store(page)
#
#     ui.login(page, user_email, password)
#
#     expect(page.get_by_role("link", name="Account Login")).not_to_be_visible()
#     expect(page.get_by_role("link", name="Account Account")).to_be_visible()
#     expect(page.get_by_role("link", name="Logout")).to_be_visible()
