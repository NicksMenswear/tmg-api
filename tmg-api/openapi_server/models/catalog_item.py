from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from openapi_server.models.base_model import Model
from openapi_server import util


class CatalogItem(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, id=None, name=None, description=None, price=None):  # noqa: E501
        """CatalogItem - a model defined in OpenAPI

        :param id: The id of this CatalogItem.  # noqa: E501
        :type id: int
        :param name: The name of this CatalogItem.  # noqa: E501
        :type name: str
        :param description: The description of this CatalogItem.  # noqa: E501
        :type description: str
        :param price: The price of this CatalogItem.  # noqa: E501
        :type price: float
        """
        self.openapi_types = {
            'id': int,
            'name': str,
            'description': str,
            'price': float
        }

        self.attribute_map = {
            'id': 'id',
            'name': 'name',
            'description': 'description',
            'price': 'price'
        }

        self._id = id
        self._name = name
        self._description = description
        self._price = price

    @classmethod
    def from_dict(cls, dikt) -> 'CatalogItem':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The CatalogItem of this CatalogItem.  # noqa: E501
        :rtype: CatalogItem
        """
        return util.deserialize_model(dikt, cls)

    @property
    def id(self) -> int:
        """Gets the id of this CatalogItem.


        :return: The id of this CatalogItem.
        :rtype: int
        """
        return self._id

    @id.setter
    def id(self, id: int):
        """Sets the id of this CatalogItem.


        :param id: The id of this CatalogItem.
        :type id: int
        """

        self._id = id

    @property
    def name(self) -> str:
        """Gets the name of this CatalogItem.


        :return: The name of this CatalogItem.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name: str):
        """Sets the name of this CatalogItem.


        :param name: The name of this CatalogItem.
        :type name: str
        """
        if name is None:
            raise ValueError("Invalid value for `name`, must not be `None`")  # noqa: E501

        self._name = name

    @property
    def description(self) -> str:
        """Gets the description of this CatalogItem.


        :return: The description of this CatalogItem.
        :rtype: str
        """
        return self._description

    @description.setter
    def description(self, description: str):
        """Sets the description of this CatalogItem.


        :param description: The description of this CatalogItem.
        :type description: str
        """

        self._description = description

    @property
    def price(self) -> float:
        """Gets the price of this CatalogItem.


        :return: The price of this CatalogItem.
        :rtype: float
        """
        return self._price

    @price.setter
    def price(self, price: float):
        """Sets the price of this CatalogItem.


        :param price: The price of this CatalogItem.
        :type price: float
        """
        if price is None:
            raise ValueError("Invalid value for `price`, must not be `None`")  # noqa: E501

        self._price = price
